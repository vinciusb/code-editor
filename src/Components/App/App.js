...
function App() {
    ...
    function getSourceCode(code, id) {
        var newCode = sourceCodes.map((element, i) => {
            return i === id ? code : element;
        });
    ...
    function renderCodeEditors() {
        ...
        props.forEach((obj, i) => {
            list.push(<CodeEditor key={ i } id={ i } lang={ obj.lang } logo={ obj.logo }
                handleCodeTransfer={ getSourceCode } shouldTransfer={ isTransferCode } />);
        });
